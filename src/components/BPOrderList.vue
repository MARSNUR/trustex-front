<template>
  <div class="container">
    <header class="mt-5">
      <div class="row">
        <div class="col-6">
          <h3>Список моих переводов</h3>
        </div>
        <div class="col-6 text-right">
          <router-link class="btn btn--sm btn--main" to="/bp-order-create">
            <svg class="btn__icon">
            ...
            <div class="orders-table__item" v-for="item in orders" :key="item.userOrderP2PId">
              ...
              <div class="orders-table__row">
                ...
                <div class="orders-table__cell  orders-table__cell--datetime text-center">
                  ...
                  <button class="btn btn--sm btn--danger" v-if="item.apply === false && item.cancel === false" type="button" @click="handleCancelModalOpen(item)">Отменить</button>
                    ...
                    <div class="order-page__main-data  data-group-row">
                      ...
                      <div class="data-group-row__item  data-group">
                        ...
                        <div class="pt-2 justify-content-end" v-if="item.apply === false && item.cancel === false">
                          <button class="btn btn--sm btn--danger" type="button" @click="handleCancelModalOpen(item)">Отменить</button>
                        </div>
        ...
        <div class="modal-container">
          ...
          <div class="modal-body">
            ...
            <div class="pb-3" v-else>
              <h1> Вы действительно хотите отменить перевод? </h1>

              <div class="data-group-row__item  data-group">
                <div class="data-group__label">Отправитель</div>
                <div class="data-group__val"><span class="color-grey-light">ФИО: </span> {{ itemForCancel.userFromMoney.fio }}</div>
              </div>

              <div class="data-group-row__item  data-group">
                <div class="data-group__label">Получатель</div>
                <div class="data-group__val"><span class="color-grey-light">ФИО: </span> {{ itemForCancel.userToMoney.fio }}</div>
                <div class="data-group__val"><span class="color-grey-light">Телефон: </span> {{ itemForCancel.userToMoney.phoneNumber }}</div>
              </div>
            </div>

            <div class="row">
              <div class="col-6">
                <button class="btn  btn--full-width  btn--main" type="button" @click="handleCancelModalClose" :disabled="modalLoading">
                  Отмена
                </button>
              </div>
              <div class="col-6">
                <button class="btn  btn--full-width  btn--danger" type="button" @click="handleCancelModalConfirm" :disabled="modalLoading">
...
export default {
  ...
  methods: {
    loadOrderList(userId, apply, cancel){
      this.orders = []
      BpService.getOrdersList(userId, apply, cancel).then(
          (response) => {
    ...
    handleFilterChange(){
      if (this.filter === "1"){
        this.loadOrderList(this.currentUser.userId, false, false)
      }
      if (this.filter === "2"){
        this.loadOrderList(this.currentUser.userId, true, false)
      }
      if (this.filter === "3"){
        this.loadOrderList(this.currentUser.userId, false, true)
      }
    },

    handleCancelModalOpen(item){
      this.showCancelOrderModal = true
      this.itemForCancel = item
    },

    handleCancelModalConfirm(){
      this.modalLoading = true
      BpService.cancelBPOrder(this.itemForCancel.id).then(
          (response) => {
            console.log(response)
            createToast("Перевод успешно отменен!", {
              type: 'success',
              position: 'top-right',
              showIcon: true,
            });
